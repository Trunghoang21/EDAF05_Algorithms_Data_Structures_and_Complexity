...
class Graph:
    def __init__(self, nbr_nodes, nbr_edges):
        ...
        self.graph = [[0] * nbr_nodes for _ in range(nbr_nodes)] # list of lists to represent the graph
    ...
    def jarnik(self, start_node = 1):
        ...
        for i in range(self.nbr_nodes):
            if self.graph[start_node][i] != 0:
                hq.heappush(prioryty_queue, (self.graph[start_node][i], (start_node, i))) # heapush adds the edge to the priority queue
            if self.graph[i][start_node] != 0:
                hq.heappush(prioryty_queue, (self.graph[i][start_node], (start_node, i)))
            ...
            if edge[1] not in visted:
                ...
                for i in range(self.nbr_nodes):
                    if self.graph[edge[1]][i] != 0 and i not in visted:
                        hq.heappush(prioryty_queue, (self.graph[edge[1]][i], (edge[1], i)))
                # add the edge to the priority queue.
                for i in range(self.nbr_nodes):
                    if self.graph[i][edge[1]] != 0 and i not in visted:
                        hq.heappush(prioryty_queue, (self.graph[i][edge[1]], (edge[1],i)))                
...
def main(): 
    ...
    # 
    #print(f"the first line is {nbr_nodes} {nbr_edges}")
    # initilize the graph 
